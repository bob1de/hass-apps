...
class HeatyApp(common.App):
    ...
    def initialize_inner(self) -> None:
        ...
        self.log(
            "Heaty is deprecated in favour of Schedy. Please consult "
        ...
        self.log("Importing modules for use in temperature expressions.", level="DEBUG")
        ...
        self.log("Listening for heaty_reschedule event.", level="DEBUG")
        self.listen_event(
            self._reschedule_event_cb, "heaty_reschedule", **heaty_id_kwargs
        )

        self.log("Listening for heaty_set_temp event.", level="DEBUG")
        self.listen_event(self._set_temp_event_cb, "heaty_set_temp", **heaty_id_kwargs)

        master = self.cfg["master_switch"]
        if master:
            self.log(
                "Listening for state changes of master switch "
                "(entity_id={}).".format(repr(master)),
                level="DEBUG",
            )
        ...
        if self.master_is_on():
            ...
            self.log("Master switch is off, not setting temperatures " "initially.")
    ...
    def _master_switch_cb(
        ...
        self.log(
            "Master switch turned {}.".format(new), prefix=common.LOG_PREFIX_INCOMING
        )
        for room in self.rooms:
            if new == "on":
                room.apply_schedule()
    ...
    def _reschedule_event_cb(self, event: str, data: dict, kwargs: dict) -> None:
        ...
        if data.get("heaty_id", self.cfg["heaty_id"]) != self.cfg["heaty_id"]:
            self.log(
                "Ignoring re-schedule event for heaty_id '{}', "
                "ours is '{}'.".format(data.get("heaty_id"), self.cfg["heaty_id"]),
                level="DEBUG",
            )
            return
        ...
        if room_name:
            ...
            if not room:
                self.log(
                    "Ignoring heaty_reschedule event for "
        ...
        self.log(
            "Re-schedule event received for: {}{}.".format(
    ...
    def _set_temp_event_cb(self, event: str, data: dict, kwargs: dict) -> None:
        ...
        if data.get("heaty_id", self.cfg["heaty_id"]) != self.cfg["heaty_id"]:
            self.log(
                "Ignoring set_temp event for heaty_id '{}', "
                "ours is '{}'.".format(data.get("heaty_id"), self.cfg["heaty_id"]),
                level="DEBUG",
            )
            return
        ...
        try:
            ...
            if not isinstance(reschedule_delay, (type(None), float, int)):
                raise TypeError()
            if isinstance(reschedule_delay, (float, int)) and reschedule_delay < 0:
                raise ValueError()
        except (KeyError, TypeError, ValueError):
            self.log(
                "Ignoring heaty_set_temp event with invalid data: {}".format(
                    repr(data)
                ),
                level="WARNING",
            )
        ...
        if not room:
            self.log(
                "Ignoring heaty_set_temp event for unknown "
        ...
        if (
            not self.cfg["untrusted_temp_expressions"]
            and expr.Temp.parse_temp(temp_expr) is None
        ):
            self.log(
                "Ignoring heaty_set_temp event with an "
    ...
    def require_master_is_on(self) -> bool:
        ...
        if not self.master_is_on():
            ...
            self.log(
                "Master switch is off, aborting {}.".format(repr(caller_name)),
                level="DEBUG",
            )
            return False
        return True
