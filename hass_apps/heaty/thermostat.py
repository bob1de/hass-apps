...
class Thermostat:
    ...
    def _check_config_plausibility(self) -> None:
        ...
        if not required_attrs:
            self.log(
                "At least one of supports_opmodes and "
    ...
    def _state_cb(
        ...
        if target_temp != self.current_target_temp:
            if self.cfg["supports_temps"]:
                self.log(
                    "Received target temperature of {}.".format(str(target_temp)),
                    prefix=common.LOG_PREFIX_INCOMING,
                )
            else:
                self.log(
                    "Received state of {}.".format(
                        "OFF" if target_temp.is_off else "ON"
                    ),
                    prefix=common.LOG_PREFIX_INCOMING,
                )

            self.current_target_temp = target_temp
            self.events.trigger("target_temp_changed", self, target_temp)
    ...
    def initialize(self) -> None:
        ...
        self.log(
            "Initializing thermostat (entity_id={}).".format(repr(self.entity_id)),
            level="DEBUG",
        )
        ...
        if state is None:
            self.log(
                "State for thermostat is None, ignoring it for now.", level="WARNING"
            )
        else:
    ...
    def is_synced(self) -> bool:
        ...
        return (
            self.resend_timer is None
            and self.current_target_temp is not None
            and self.current_target_temp == self.wanted_temp
        )
    ...
    def set_temp(
        ...
        if target_temp.is_off:
            ...
            if (
                isinstance(self.cfg["min_temp"], expr.Temp)
                and temp < self.cfg["min_temp"]
            ):
                temp = None
                opmode = self.cfg["opmode_off"]
            else:
                ...
                if (
                    isinstance(self.cfg["max_temp"], expr.Temp)
                    and temp > self.cfg["max_temp"]
                ):
        ...
        if not self.cfg["supports_opmodes"]:
            if opmode == self.cfg["opmode_off"]:
                ...
                if self.cfg["min_temp"] is not None:
                    self.log(
                        "Setting to minimum supported temperature " "instead.",
                        level="DEBUG",
                    )
        ...
        if not self.cfg["supports_temps"] and temp is not None:
            self.log(
                "Not setting temperature because thermostat doesn't "
        ...
        if opmode is None and temp is None:
            self.log("Nothing to send to this thermostat.", level="DEBUG")
            return None
        ...
        if not force_resend and self.is_synced:
            self.log("Not sending temperature redundantly.", level="DEBUG")
            return None
    ...
    def _set_temp_resend_cb(self, kwargs: dict) -> None:
        ...
        self.log(
            "Setting temperature = {}, operation mode = {}, "
            "left retries = {}.".format(
                temp or "<unset>", repr(opmode) or "<unset>", left_retries
            ),
            level="DEBUG",
